...
suite('Functional Tests', function() {
  ...
  test('Convert a valid input such as 10L', function(done) {
    chai.request(server)
      ...
      .end(function(err, res) {
        ...
        assert.approximately(res.body.returnNum, 2.64172, 0.1); // 10 liters to gallons
  ...
  test('Convert with no number such as kg', function(done) {
    chai.request(server)
      ...
      .end(function(err, res) {
        assert.equal(res.status, 200);
        assert.equal(res.body.initNum, 1); // Default value for no number input
        assert.equal(res.body.initUnit, 'kg');
        assert.approximately(res.body.returnNum, 2.20462, 0.1); // 1 kilogram to pounds
